...
void hw_rev_2_SingleLidarOpenRound::init(ILogger* logger) {
  ...
  // Set stopping distance based on front LiDAR distance after final turn
...
VehicleCommand hw_rev_2_SingleLidarOpenRound::drive(VehicleData vehicleData){
  ...
  if (turning == true){
    if ((turns == 11) and (abs(difference) < 20) and (stopDist == 0)){ // To fix stopping after final turn
      speed = 0;
      distance = 0;
      pos = 90;
      encoderValue = 0;
  ...
  if (turns == 12){
    ...
    else speed = 190; // Slow down before stopping
  }
