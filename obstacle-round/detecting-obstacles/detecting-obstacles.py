    ...
    for cnt in contours:
        if cv2.contourArea(cnt) > min_area and cv2.contourArea(cnt) < max_area:
            ...
            if h > w:
                ...
                row = 0
                col = 0
                obs.append([(x,y,w,h), (row, col)])
    return obs


def decide_path(red_obs, green_obs):
    # Needs to be updated
    # Grid positions occupied by obstacles
    red_cells = [pos[1] for pos in red_obs]
    green_cells = [pos[1] for pos in green_obs]
...
def nearest_obstacle():
    global red_obs, green_obs
    nearest_obs = [(590,0,100,150),'colour']
    for obs in red_obs: 
